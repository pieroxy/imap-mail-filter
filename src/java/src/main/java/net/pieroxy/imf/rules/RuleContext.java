 ...
 * replacing what used to be a per-thread ThreadLocal); expected to grow — e.g. the account's own
 * address, once REPLY/FORWARD actions exist and need it.
 */
public record RuleContext(File subjectModelFile, File headerModelFile, File bodyModelFile) {
  /** No account-specific info available — matches how things behaved before this existed. */
  public static final RuleContext EMPTY = new RuleContext(null, null, null);
}
